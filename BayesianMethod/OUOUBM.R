...
NegLogLike<-function(x,regb=regb,n=n,G=G,distG=distG,one=one,response=response,pred.dataset=pred.dataset,dsm=dsm){
  ...
  if(alp<0 ||alp>100||bet<0 ||bet>100|| alp==bet ||xi<0 || !is.finite(negloglike) || negloglike <= -1000){
    return(badval)
  }
  return(negloglike[1])
}#Neglog
...
sim.dsm.ouou<-function(model.params,pred.dataset=pred.dataset,x.tre=x.tre){
  ...
  one<-array(1,c(n,1))
  #print(one)
  second<-phyloCorrectFactor.OUOUBM(c(alp,bet,xi),pred.dataset = pred.dataset,x.tre=x.tre)*pred.dataset
  return(second)
}
#-------------------------------------------------------------------------------
phyloCorrectFactor.OUOU<-function(model.params, pred.params=pred.params, pred.dataset=pred.dataset,x.tre=x.tre){
  ...
  cov_y_th<-
    -((alp - bet)*y_0*exp(bet*trh) + (alp*exp(alp*trh) - alp*exp(bet*trh))*th_0)*th_0*exp(-alp*trh - 2*bet*trh)/(alp - bet) + 1/2*(2*(alp^2*bet - bet^3)*th_0*y_0*exp(alp*trh + bet*trh) + (alp^2 - alp*bet)*sig_th^2 - 2*(alp^2*bet - alp*bet^2 - (alp^2*bet - alp*bet^2)*exp(alp*trh + bet*trh))*th_0^2 + 2*(alp*bet*sig_th^2 + (alp*bet + bet^2)*sig_th*rho_y_th*sig_0)*exp(alp*trh + bet*trh) - (2*(alp*bet + bet^2)*sig_th*rho_y_th*sig_0 + (alp^2 + alp*bet)*sig_th^2)*exp(2*bet*trh))*exp(-2*bet*trh)/(alp^2*bet - bet^3)
  var_th<-
    -1/2*bet*sig_th^2*(exp(-2*bet*trh) - 1)*exp(-bet*0)
...
prior<-function(model.params, pred.params){
  ...
  return(alpprior + betprior + xiprior + predprior)
}
...
proposalfunction<-function(model.params, pred.params){
  ...
  return(c(abs(pps.alp), abs(pps.bet), abs(pps.xi), pps.pred.params))
}
...
startvalue<- c(alpha0,beta0,xi0,b_ini)
...
hist(chain[-(1:burnIn),3],nclass=30, main="Posterior of xi", xlab= " True value = red line")
