GlobalOptim<-function(NegLogLike,CovRes,precision=precision,p0=p0,regb=regb,n=n,G=G,distG=distG,one=one,response=response,pred.dataset=pred.dataset,dsm=dsm,model=model){
  ...
  # p0<- c(2,3,4,1,1,2,5)
  #regb<-b_ini
  #best.run<-list(value=NegLogLike(p0,regb=regb,n=n,G=G,distG=distG,one=one,response=response,pred.dataset=pred.dataset,dsm=dsm),par=p0)
  best.run<-list(value=NegLogLike(p0,regb=regb,n=n,G=G,distG=distG,one=one,response=response,pred.dataset=pred.dataset,dsm=dsm),par=p0)
  ...
  while(trial <=1){
    ...
    while(tryout<=3){
      ...
      #print(p0)
      if(model=="OUBM"){
        dsm<-sim.dsm.oubm(p0,pred.dataset=pred.dataset,x.tre=x.tre)
        new.run<-optim(p0,NegLogLike,method="Nelder-Mead",regb=regb,n=n,G=G,distG=distG,one=one,response=response,pred.dataset=pred.dataset,dsm=dsm)       
      }
      if(model=="OUOU"){
        dsm<-sim.dsm.ouou(p0,pred.dataset=pred.dataset,x.tre=x.tre)
        new.run<-optim(p0,NegLogLike,method="Nelder-Mead",regb=regb,n=n,G=G,distG=distG,one=one,response=response,pred.dataset=pred.dataset,dsm=dsm)       
      }
      if(model=="OUBMBM"){
        dsm<-sim.dsm.oubmbm(p0,pred.dataset=pred.dataset,x.tre=x.tre)
        new.run<-optim(p0,NegLogLike,method="Nelder-Mead",regb=regb,n=n,G=G,distG=distG,one=one,response=response,pred.dataset=pred.dataset,dsm=dsm)       
      }
      if(model=="OUOUBM"){
        dsm<-sim.dsm.ououbm(p0,pred.dataset=pred.dataset,x.tre=x.tre)
        new.run<-optim(p0,NegLogLike,method="Nelder-Mead",regb=regb,n=n,G=G,distG=distG,one=one,response=response,pred.dataset=pred.dataset,dsm=dsm)       
      }
      if(model=="OUBMCIR"){
  ...
  if(model=="OUBM"){
    dsm<-sim.dsm.oubm(best.run$par,pred.dataset=pred.dataset,x.tre=x.tre)
    est_V<- CovRes(best.run$par,n=n,G=G,distG=distG,one=one,response=response,pred.dataset=pred.dataset)
  }
  if(model=="OUOU"){
    dsm<-sim.dsm.ouou(best.run$par,pred.dataset=pred.dataset,x.tre=x.tre)
    est_V<- CovRes(best.run$par,n=n,G=G,distG=distG,one=one,response=response,pred.dataset=pred.dataset)
  }
  if(model=="OUBMBM"){
    dsm<-sim.dsm.oubmbm(best.run$par,pred.dataset=pred.dataset,x.tre=x.tre)
    est_V<- CovRes(best.run$par,n=n,G=G,distG=distG,one=one,response=response,pred.dataset=pred.dataset)
  }
  if(model=="OUOUBM"){
    dsm<-sim.dsm.ououbm(best.run$par,pred.dataset=pred.dataset,x.tre=x.tre)
    est_V<- CovRes(best.run$par,n=n,G=G,distG=distG,one=one,response=response,pred.dataset=pred.dataset)
  }
  if(model=="OUBMCIR"){
    dsm<-sim.dsm.oubmcir(best.run$par,predictor_1=predictor_1,predictor_2=predictor_2,x.tre=x.tre)
