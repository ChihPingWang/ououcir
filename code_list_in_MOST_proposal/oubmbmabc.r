...
oubmbmprior<-function(prior.model.params=prior.model.params,prior.reg.params=prior.reg.params){
  alpha.y.rate <-prior.model.params[1]
  sigma.sq.x.shape <-prior.model.params[2]
  sigma.sq.x.scale <-prior.model.params[3]
  tau.shape <- prior.model.params[4]
  tau.scale <- prior.model.params[5]
  
  alpha.y<-rexp(n=1, rate=alpha.y.rate)
  sigma.sq.x<-rinvgamma(n=1, shape=sigma.sq.x.shape, scale=sigma.sq.x.scale)
...
prior.model.params=c(alpha.y.rate, sigma.sq.x.shape, sigma.sq.x.scale, tau.shape, tau.scale)
names(prior.model.params)<-c("alpha.y.rate", "sigma.sq.x.shape", "sigma.sq.x.scale", "tau.shape", "tau.scale")
prior.reg.params=c(b0.min, b0.max, b1.min, b1.max, b2.min, b2.max)
# alpha.y <- rexp(n=1, rate=5);alpha.y
# sigma.sq.x <- rinvgamma(n=1, shape=2, scale=1);sigma.sq.x
...
rownames(model.params.array)<-c("alpha.y","sigma.sq.x","tau")
...
post.model.params.array<-array(0,c(3,sims))
rownames(post.model.params.array)<-c("alpha.y","sigma.sq.x","tau")
...
oubmbm.post.sim <- cbind(t(post.model.params.array),t(post.reg.params.array))
